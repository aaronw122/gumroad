...
module StripeMerchantAccountManager
  ...
  def self.handle_stripe_info_requirements(stripe_event_id, stripe_account, stripe_previous_attributes)
    ...
    sync_external_accounts_from_stripe(user, merchant_account, stripe_account)
  ...
  def self.sync_external_accounts_from_stripe(user, merchant_account, stripe_account)
    ...
    user.bank_accounts.alive
      ...
      .where.not(stripe_bank_account_id: incoming_external_ids)
    ...
    external_accounts.each do |external_account|
      ...
      if external_account["object"] == "card"
        sync_card_external_account(user, stripe_account_id, external_account)
      elsif external_account["object"] == "bank_account"
        sync_bank_external_account(user, stripe_account_id, external_account)
      end
    end
  rescue => e
    ErrorNotifier.notify("Failed to sync external accounts from Stripe for user #{user.id}: #{e.message}")
  end

  private_class_method
  def self.sync_bank_external_account(user, stripe_account_id, external_account)
